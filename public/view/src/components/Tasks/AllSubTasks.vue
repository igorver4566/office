    ...
    >
      <template slot="items" slot-scope="props">
        <tr :key="props.item.id">
          ...
          <td @click="props.expanded = !props.expanded">{{ props.item.task_name }}</td>
...
export default {
  ...
  methods: {
    ...
    edit (key) {
      var tasks = this.$store.getters.subTasks
  ...
  computed: {
    tasks () {
      var task = this.$store.getters.subTasks
      var taskNames = this.$store.getters.tasks
      var uName = this.$store.getters.userName
      if (task !== null && uName !== '') {
        if (!uName === 'Администратор') {
          task = task.filter(el => el.user === uName)
        }
        task = task.map(
          el => {
            el.task_name = taskNames.find(
              elem => {
                if (elem.id === el.task_id) {
                  return true
                }
                return false
              }
