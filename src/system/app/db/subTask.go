...
func GetAllSubTasks() ([]SubTaskReturn, error) {
	...
	d := Init()
	defer d.Close()
	rows, err := d.Query("SELECT s.id, s.name, s.price, s.time, s.message, s.task_id, st.name, u.name FROM sub_task s INNER JOIN status st ON s.status_id = st.id INNER JOIN users u ON s.user_id = u.id ORDER BY s.id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		err := rows.Scan(&subTask.ID, &subTask.Name, &subTask.Price, &subTask.Time, &subTask.Message, &subTask.TaskID, &subTask.Status, &subTask.User)
		if err != nil {
			return nil, err
		}
